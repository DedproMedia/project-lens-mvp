...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setVisible(config.visibility || {});
      setEditable(config.editability || {});

      // Load clients
      const { data: cs } = await supabase.from("clients").select("id,name").order("name");
      if (cs) setClients(cs.map((c) => ({ id: String(c.id), name: c.name })));
    };
              ...
              <div className="absolute top-2 right-2 flex gap-2 items-center">
                <label className="flex items-center gap-1 text-xs">
                  <input
                    ...
                    onChange={(e) => setVisible({ ...visible, [el]: e.target.checked })}
                  />
                  Visible
                </label>
                <label className="flex items-center gap-1 text-xs">
                  <input
                    ...
                    onChange={(e) => setEditable({ ...editable, [el]: e.target.checked })}
                  />
                  Editable
                </label>
                <button
