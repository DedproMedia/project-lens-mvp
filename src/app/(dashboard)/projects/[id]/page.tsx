...
export default function ProjectDetailPage() {
  ...
  const [active, setActive] = useState<Record<ElementKey, boolean>>(
    () =>
      Object.fromEntries(ALL_ELEMENTS.map((k) => [k, false])) as Record<
        ElementKey,
        boolean
      >
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const a = Object.fromEntries(
        ALL_ELEMENTS.map((k) => [k, Boolean(cfg.elements?.includes(k))])
      ) as Record<ElementKey, boolean>;
      const v = Object.fromEntries(
        ALL_ELEMENTS.map((k) => [k, Boolean(cfg.visibility?.[k])])
      ) as Record<ElementKey, boolean>;
      const e = Object.fromEntries(
        ALL_ELEMENTS.map((k) => [k, Boolean(cfg.editability?.[k])])
      ) as Record<ElementKey, boolean>;
      setActive(a);
      setVisible(v);
      setEditable(e);
      ...
      setData({
        ...d0,
        "project_status.name": d0["project_status.name"] ?? "",
        "project_status.custom": d0["project_status.custom"] ?? "",
      });
  ...
  const selectedElements = useMemo(
    () => ALL_ELEMENTS.filter((k) => active[k]),
    [active]
  );
  ...
  const save = async () => {
    ...
    // persist config (correctly map visible/editable into visibility/editability)
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1100 }}>
        ...
        {editMode ? (
          ...
          >
            ...
            <input
              ...
              style={{ minWidth: 240 }}
            />
            <Link href="/settings/status-types" style={{ marginLeft: 8 }}>
              Manage status list →
            </Link>
          </div>
        ) : (
          <KV
            label="Status"
            value={
              getD<string>("project_status.custom", "") ||
              getD<string>("project_status.name", "—")
            }
          />
        )}
      </Section>
            ...
            {ALL_ELEMENTS.map((k) => (
              <label key={k} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input
                  type="checkbox"
                  checked={!!active[k]}
                  onChange={() => toggle(active, setActive, k)}
                />
                ...
                <tr key={k} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  ...
                  <td style={{ padding: 8 }}>
                    <input
                      type="checkbox"
                      checked={!!visible[k]}
                      onChange={() => toggle(visible, setVisible, k)}
                    />
                  </td>
                  <td style={{ padding: 8 }}>
                    <input
                      type="checkbox"
                      checked={!!editable[k]}
                      onChange={() => toggle(editable, setEditable, k)}
                    />
                  </td>
                </tr>
              ))}
      ...
      {/* Keep your other element editors below if you had them */}
    </div>
  );
}
...
function labelFor(key: ElementKey) {
  switch (key) {
    ...
    case "deliverables":
      return "Deliverables";
    case "project_dates_locations":
      return "Project Date & Location";
    case "project_status":
